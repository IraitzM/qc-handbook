...
So, now you know how a simulator works. It essentially performs the mathematical operations we expect a quantum computer will do, but there are quite some nuances on the way in which we do this. For example, using a _simulator_ is not the same as using an _emulator_.

A _simulator_ performs specifically the actions of the mathematical framework, without any restriction or special case. Simply put, it is what the ideal machine should do. An _emulator_ on the other hand, performs the same set of actions but considering some of the limitations a particular device could have. Essentially it is trying to show what the same actions performed by a physical machine would look like.

Then of course, there is sending the job to a real device through a service like the ones offered by the [IBM Quantum Platform](https://quantum.cloud.ibm.com/). You will need to create an account but after that, you can submit all the circuits you want to real hardware. Check out [this example](../appendix/submit_circuit.ipynb).

## Simulators

The actions we performed in the [previous exercise](simulations.qmd) were mimicking the state vector of our system and performing the unitary operations that drive the state. This is often known as **State Vector** simulator (dah!) and it is common on some of the most used SDKs. The ability to simulate a given circuit is proportional to the resources needed to manipulate that state vector.
...
Qiskit is not the only SDK we will be using so it is good to know these very same simulation techniques exist when, for example, using [Pennylane](https://pennylane.ai/) you will have access to several [devices](https://pennylane.ai/devices/) including:
...
In any case, this is not a trivial task. In fact, some have already claimed it to be a Herculean task [@xu2023herculeantaskclassicalsimulation]. There is a useful guide by Google on [how to choose your hardware](https://quantumai.google/qsim/choose_hw). Let's keep it simple for now and deal with algorithm implementations.
