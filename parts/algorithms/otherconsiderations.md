...
So, if we take the example of the portfolio optimization problem, assuming a percentage of the available money goes to different assets (not just binary decisions if we invest or not) we will need to reformulate it, so imagine that instead of
...
The limitation to quadratic terms might not be suitable for all sorts of problems. We may want to extend our problem so that it covers, for example, a three-party relationship

$$
\max \sum_i a_ix_i - \sum_i \sum_j b_{ij}x_i, x_j + \sum_i \sum_j \sum_k c_{ijk}x_ix_jx_k.
$$

That extra three-party relation adds a whole new level of complexity as it requires a three-party gate (like the Toffoli gate) in systems that do not go beyond two-qubit gates. So we will need to decompose this interaction into several new two-body terms, exploding the resources required. These are defined as [Higher-Order Unconstrained Binary Optimization (HUBO) problems](https://docs.dwavequantum.com/en/latest/quantum_research/reformulating.html#non-quadratic-higher-degree-polynomials).