...
For example, if our problem has a convex shape (meaning it looks like a bowl), then there is an entire field dedicated to it.

* [Convex Optimization](https://en.wikipedia.org/wiki/Convex_optimization): studies the case when the objective function is convex (minimization) or concave (maximization) and the constraint set is convex. This can be viewed as a particular case of nonlinear programming or as a generalization of linear or convex quadratic programming.
...
* [Binary Optimization](https://en.wikipedia.org/wiki/Integer_programming) (BO) is a special case of Integer programming where only two values are allowed for our variables, 0 or 1, hence the name binary.

These last problems often require a relaxed version of the problem where we solve using solvers suited for previous optimization techniques and then we iterate adding restrictions to approximate our goal. This technique is known as [Branch-and-Bound](https://en.wikipedia.org/wiki/Branch_and_bound).
...
Binary optimization is a challenging task and quantum computers took a chance at it, as binary problems already fall in their nature.