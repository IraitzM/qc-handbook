...
One relevant aspect of inequalities is that we will need to add slack variables to take the excess of the constraint $a_j x_j + S = b $. In this case $S$ will be a value between 0 and $b$ so we would need to characterize its binary representation so that the problem is still a binary quadratic problem ($S = 2^ns_n + \dots + 2^0s_0$)
...
Consider that our QUBO is symmetric, then each penalty should be divided by two if it occupies $Q_{i,j}$ and $Q_{j,i}$ positions in our matrix. That way we have filled our matrix with good and bad choice penalties, considering it is better to stay in a place than being in two places at once or considering two trajectories for the same period.
