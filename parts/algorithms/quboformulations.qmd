...
Usually a good estimate for a Lagrange parameter is between 75-150% of the objective function value, so we come up with an estimate for tour length and use that.
...
* We implement the penalty as: $$\text{Penalty}_n = \lambda\left(\sum_{p=0}^{N-1} x_{n,p} - 1\right)^2$$ indicating that a single occupied variable should happen per node at a high cost ($\lambda$) for its potential positions ($p$ here as a general approach to the time indexed $it$).
...
for node in G:
    for pos_1 in range(N):
        ...
        for pos_2 in range(pos_1+1, N):
            Q[((node, pos_1), (node, pos_2))] += lagrange # Cost of not moving
