...
In our QUBO setup, this constraints need to be part of the objective function acting as a maximizer in our example. Usually a good estimate for a Lagrange parameter is between 75-150% of the objective function value, so we come up with an estimate for tour length and use that.
...
* We implement the penalty as: $$\text{Penalty}_n = \lambda\left(\sum_{p=0}^{P-1} x_{n,p} - 1\right)^2$$ indicating that a single occupied variable should happen per node at a high cost ($\lambda$) for its potential positions ($p$ here as a general approach to the time indexed $it$).
...
$$
\text{Penalty}_n = \lambda\left(-\sum_p x_{n,p} + 2\sum_{p<q} x_{n,p}x_{n,q} + 1\right)
$$

**Penalty examples:**

- Single active node:  $k=1 \rightarrow$ Penalty $=0$
- $k=0$ or $k=2 \rightarrow$ Penalty $=\lambda$
- $k=3 \rightarrow$ Penalty $=4\lambda$
...
$$
\min \sum_p\sum_i^n \sum_{j, j \ne i}^n c_{ij}x_{i,p}x_{j,(p+1)} + \sum_{n=1}^N\lambda\left(-\sum_p x_{n,p} + 2\sum_{p<q} x_{n,p}x_{n,q} + 1\right)
$$
...
Consider that our QUBO is symmetric, then each penalty should be divided by two if occupies $Q_{X_{i},X_{j}}$ and $Q_{X_{j},X_{i}}$ positions in our matrix. That way we have filled our matrix with good and bad choice penalties, considering it is better to stay in a place than being in two places at once or considering to trajectories for the same period.

Now is a matter of indicating the cost of each decision of movement.
...
Which ends up rendering the following matrix...
...
We will recover this formulation for the comparison example on the different ways of implementing the solution to this challenge.