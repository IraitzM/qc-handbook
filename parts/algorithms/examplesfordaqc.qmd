...
We will use our previous example on MaxCut just to keep it simple.
...
Ok, we don't always know the solution in advance but it is important for educational purposes. Let's take the three pieces from our digitized AQC and build block by block, starting by the quantum state that is the ground state of our $H_{init}=-\sum_n \sigma_i^n$
...
This following code is a particular version of the MaxCut instance as in a fully informed Ising model we would need to add $Z$ rotations depending on the weight of the _node_ and $ZZ$ rotations according to coupling strength of each edge. But given that we have a sparse matrix, we can take advantage of that.
...
With $n$ being the number of Trotter steps. We can do a first approach with a simple stepwise linear approach.
...
Let's play around a bit with the time parameters and see if we get a good concentration of -5 energy states. Ideally our sampling from the ground state should be below $2^5 = 32$ samples, otherwise, brute forcing would be more efficient in these cases. But in absence of knowledge we should guess for a while.
