#  QAOA in practice {.unnumbered}


As we have seen before, QAOA takes the best of both worlds (ideally) considering the ansatz out of the AQC but variationally training it to obtain the best scheduling function out of the free parameters.
...
In fact, for this example we know there is more than one optimal solution...
...
In this case, we will start with the existing Qiskit functionality and do a drill-down to highlight what is done behind the scenes. Qiskit provides a ready to be used QAOA implementation that will only require an optimizer and a way to extract information out of our quantum device (either Estimator or Sampler primitive).
...
As we can see the ansatz is built following the Adiabatic Quantum Computing scheme where a block of $X$ rotations is applied, and alternated with the problem Hamiltonian ($Z + ZZ$ interactions) as many times as repetitions are performed.
...
Obtained solution distribution really points to the states that provide solutions to our problem, superposed in a state that is the one minimizing the outcome of the circuit with respect to the expectation value of the Hamiltonian.
...
Therefore, we are able to encode our problem into an Ising Hamiltonian, so that we can _easily_ design a circuit to obtain the results from the potential solution landscape. We may need to set up the repetitions needed in order to map the optimal parameters to our variationally trained scheduling function.

```{python}
qaoa.ansatz.depth()
```

Small problems will render shallow circuits, but we need to be aware that introducing many repetitions of those blocks or _layers_, it will require more operations to be squeezed in, the lifespan of the qubits will need to be longer and the more operations we add, the more noise will enter into our final state.
