...
The adoption of these techniques led to new problems as well. Similar to Neural Networks, challenges when scaling the training of those techniques at larger sizes came in the shape of _barren plateaus_ [@mcclean2018barren], regions in the solution space where gradient based approaches cannot be provided with a direction in which the problem could be optimized. This also results in a high locality in certain instances that may make our variational training fall into local minima without the ability to get out of those.
...
The hardware-efficient ansatz is one of those device-aware techniques that tries to minimize the overhead when going from the theoretical representation to the practical implementation [@leone2024practical]. By already considering the device topology, one can design an ansatz that is suited to the device more than the problem itself.

Adapt-VQE [@grimsley2019adaptive] is a modification of the VQE algorithm making use of a pool of operators. It evaluates their contribution in finding the lowest energy state (_ground state_) by measuring the gradients (or contribution). The trick here is that the ansatz keeps changing from iteration to iteration looking for a specific set of operators that benefits the convergence towards the ground state. A simple example on how to use it in Qiskit can be found [https://qiskit.org/ecosystem/nature/howtos/adapt_vqe.html](https://qiskit.org/ecosystem/nature/howtos/adapt_vqe.html)

<figure markdown>
![Adapt-VQE](../../assets/adapt-vqe.png){width=60%}
</figure>

Tetris-VQE [@anastasiou2024tetris] defines an even more complex setup built on top of the previous method. By growing the ansatz evaluating the gradient but also accounting for the free space available in the remaining qubits, Tetris-VQE generated ansätze tend to be much more compact than Adapt-VQE ones.

## QAOA

Like VQE, QAOA has been put to a test and several _improvements_ have been proposed to tackle detected inefficiencies or simply try to boost its performance.

**W**arm-**S**tarted QAOA [@egger2021warm] proposes classically solving the relaxed version of the problem so that it is used to warm-start the problem (replacing our initial state towards the one of the relaxed solution). An interesting guide can be found here [https://qiskit.org/ecosystem/optimization/tutorials/10_warm_start_qaoa.html](https://qiskit.org/ecosystem/optimization/tutorials/10_warm_start_qaoa.html).
...
So far, we were able to tackle small-_ish_ problems, we could tackle larger problems of course but there are certain limitations due to the number of qubits available in the devices (in the range of 127-156 for IBM, for example).

The main challenge will come from the hardware implementation and availability of devices capable of mapping the problem to their actual chip architecture. In most cases we have talked about interacting terms between all qubits in our system but in fact, depending on the device we use to solve our problem, we might find not all qubits are connected to each other. This is the common case for most of the superconducting chips. That forces us to find ways around those limitations when pairing our problem to specific hardware. 
...
In general, the systems we often work with have a hard limitation on how big our problems can be. Some options have appeared but classical solvers can also handle small-size problems pretty well so we may find there is no huge gain in many cases. Nevertheless, we will explore what can be done.