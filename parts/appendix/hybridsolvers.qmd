...
Think about Shor's algorithm. It revolutionized the field and worried a couple of CSO/CISOs along the way. But it only changes one little step from the conventional Prime Factorization algorithm. Thanks to that step there is a significant speedup in the algorithm taking it down from exponential order to polynomial one.
...
Our approach to optimization is already limited to the QUBO formulation but this is an imposed restriction given our hardware. We may want to find solutions to problems whose variables can be integers, for example. These types of problems are classically solved by using _branch and bound_ strategies.  Taking a problem of the shape
...
where $f$ and $c$ are nonlinear and non-convex functions. This makes our target problem a non-convex mixed-integer nonlinear problem (MINLP). Having a guarantee that we not only reach a solution but **the** solution is indeed a demanding task.
...
As you can see, there is no silver bullet, but being aware of all the possibilities technology brings can make a difference when facing our business use cases for that extra percentage that makes us competitive.

### Quantagonia

Due to their change in pricing strategy, we can no longer mess around with their devices as much as we would like to so, an interesting alternative is the service provided by [Quantagonia](https://www.quantagonia.com/).
