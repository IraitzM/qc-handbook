...
Superconducting qubits are implemented using macroscopic elements built using an LC electrical circuit. The effect of an electrical resonator is observed at specific work temperatures below 15mK which requires building quite large dilution refrigerators. One of the main disadvantages of this technology is their susceptibility to environmental noise (sources being dielectric of surrounding metal or surrounding energy radiations) that cause short coherence times.

<figure markdown>
  ![Superconducting](../../assets/superconducting-qubit.gif)
</figure>

Probably one of the most mature implementations when it comes to qubit realization. Companies like IBM, Intel, Google or D-Wave base their devices on this technology. The most popular qubit, the transmon, offers coherence times between 50 and 100 $\mu$s with operation times in the order of nanoseconds (fastest: 18 ns demonstration in 2019). This leaves an operation ratio of $10^{4}$ satisfying the 3rd DiVincenzo criterion. Qubit fidelities reach 99.95% with experiments reporting 99.66% for a two-qubit gate in 2016. 

It works by controlling the current flow through the superconducting circuit and those current flows can be operated using microwave pulses. Different pulse configurations (length, amplitude or phase) represent the potential digitized gates one could find.
...
These devices can handle up to 433 qubits (IBM's Osprey architecture) on their digital version but D-Wave's annealing machine has up to 8000 qubits to be used.
...
More on their current specs can be found on their sites or cloud provider sites:
...
More detailed information [thanks to Xanadu](https://pennylane.ai/qml/demos/tutorial_trapped_ions).

## Neutral atoms

Proposed around 2000, the basic concept lies in using atoms or neutral alkali metals like Rubidium (Rb) trapped in optical arrays. By laser cooling the atoms in an array to near absolute zero temperatures, traps using magnetic fields or lasers allow for rearrangement of the atoms within the lattice. The atom's own structure represents the two-level system we are looking for therefore a one-to-one mapping is done between the atom and the qubit we would like to manufacture.
...
The arrangement of atoms in any type of 2D or even 3D structures makes problem mapping and qubit coupling flexible enough so that potentially any problem may fit into those devices. The main problem comes with the realization of two-qubit gates with high enough fidelity. Single-qubit operations reach 99.9% fidelity while two-qubit gates have been extensively studied boosting the initial 80% achieved in early 2016 to the reported 96.5% in 2019 [@levine2019parallel]. This is still an ongoing effort that limits the operability and universality of coding as most of the platforms require some kind of analog encoding as opposed to the logical abstraction others can provide.

<figure markdown>
  ![Pasqal](../../assets/pasqal.png)
</figure>

Qubit coherence times are in the order of seconds, with an operation regime of $\mu$seconds (approx. ratio $10^7$). They also have the ability to operate simultaneously in different atom clusters, enabling operation overlapping as opposed to ion-traps.
...
An easy to read and detailed vision of this technology, once again, [thanks to Xanadu](https://pennylane.ai/qml/demos/tutorial_neutral_atoms).