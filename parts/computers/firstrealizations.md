...
Motivated by Shor's algorithm [@shor1994algorithms], during the late 90s there was quite some motivation to provide a physical realization of what up to that moment was mostly theoretical work.

Any two-level physical system could be a good candidate for a qubit but we need to find a physical realization that allows for operation and performing the set of tasks we could consider an algorithm.
...
2007 is known to be a key date as the first **transmon** was born. It is the equivalent of the transistor in the quantum regime [@koch2007charge]. So the first qubits have existed since the early 2000s which is not much but from manufacturing a qubit to building a whole computer... that is a long road.
...
Also known as the **relaxation time**, identifies how much time it would take a qubit to transition from an excited state ($|1\rangle$) to its ground state ($|0\rangle$). T1 time is not an actual time from state to state but rather identifies the time in which a state might transition and flips states once excited ($P(|1\rangle) = e^{-\frac{t}{T1}}$).
...
Another measure we will often refer to is the **fidelity**. Basically it measures the gap between theoretical state and physical outcome as a qubit quality metric. Fidelity refers to the more general state of closeness between two quantum states but in the following sections it will provide a measure to understand how well each operation renders the expected result.
...
There is more than one type of quantum computer. We still don't know which one will win the podium, but the race is interesting at this point. The physical realization conditions many aspects of the whole machine so it is relevant to understand a bit of their inner workings so that we can select the most appropriate option.
...
* System must be able to be initialized ($|00...\rangle$ state is the common choice)
