...
Before going into details of what quantum kernels might be let's discuss the simplest classifier one could build. If we are provided with one number we could build a classifier saying if it is negative or positive. Or if we go to a $\mathcal{R}^2$ example we would like to know if our samples belong to the pink or blue category.
...
We would like to find the hyperplane that best splits the two groups, meaning that the _margin_ from the line to the data points is maximal. Something like
...
$$
k(x_i, x_j) = \langle 0 |\Psi^{\dagger}(x_i)\Psi(x_j)|0\rangle
$$
...
By computing the number of times we get $|0\rangle^n$ after this execution we can get the closeness of these two feature map prepared states. If orthogonal states are evaluated (ex. $\langle 0 | 1\rangle$) then we can assume their states to be as distant as possible within the Hilbert space.

This way we could easily compute the **fidelity** between states on a higher dimensional feature space by just running this exercise for each sample pair ($O(N^2)$) in what it is called a **Fidelity Kernel** [@Hubregtsen_2022].
...
<figure markdown>
![Quantum Support Vector Classifier (QSVC)](../../assets/qsvc.svg)
</figure>