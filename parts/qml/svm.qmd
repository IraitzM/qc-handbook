...
Let's see how we can compose that particular model, step by step. Probably [sQUlearn](https://squlearn.github.io/index.html) is one of the best resources in order to simplify our code. We will start loading the data. The infamous iris dataset (we will only take the first two values).
...
The task here is to find a good projection of those features so that we can better separate between the three classes (Setosa, Virginica and Versicolor). Some key steps are the splitting so that we have a fair metric to evaluate and the scaling of the variables.
...
Now we need to select a good embedding... Well, let's try with a simple Z feature map.
```{python}
...
Not bad, we could then get an estimate on the separability of a selected kernel with this. Now that we know our kernel is good enough, let's check how it works alongside a Support Vector Classifier.
...
We can try with a little bit more complex feature map. In fact, $Z$-embedding has no entanglement capacity as it is only composed of independent single-qubit rotations. We can add a little bit of entanglement to it and check if that performs better (it has entanglement and that is cool, so it should work better).
...
Curious enough, the $TA$ is higher in this case, meaning it separates better between the two groups, yet the classification is worse. Let's check out with some more complex feature map.
...
Well, the $TA$ in this case shows it separates worse in the latent [Hilbert](https://en.wikipedia.org/wiki/Hilbert_space) space data is encoded into so it might make sense that this worsens the prediction. But come on, we added entanglement, that is the magic sauce for QML. That should do its thing right? Well, I guess this is why people have not yet solved why and how QC will boost ML for the general case.